...
userRecord="$($EXEC_GETENT passwd $1)"

# Ensure the argument is a valid username
if [ ${#userRecord} -eq 0 ]; then
  printError "$SCRIPT_EXEC" "Cannot find '$1': No such user"
  echo
  printUsage "$SCRIPT_EXEC USER"
...
function installUserConfig() {
  ...
  if [ ! -f "$userhome/$configFile" ] || \
       ! $EXEC_GREP -Fq 'DevOpsBroker' "$userhome/$configFile"; then
...
excludeDirs="-type d -name '.dbus' -prune"

# Configure all normal directories with drwxr-x--- privileges
$EXEC_FIND "$userhome" $excludeDirs -o -type d -perm /027 -exec $EXEC_CHMOD 750 {} +

# Configure all hidden directories with drwx------ privileges
$EXEC_FIND "$userhome" -maxdepth 1 $excludeDirs -o -type d -name '.*' -perm /077 -exec $EXEC_CHMOD 700 {} +

printInfo "Applying stricter file security settings to $userhome"

excludeDirs="-type d ( -name '.git' -o -name '.svn' -o -name '.dbus' ) -prune"

# Remove file ----wxrwx privileges
$EXEC_FIND "$userhome" $excludeDirs -o -type f -perm /037 -exec chmod g-wx,o-rwx {} +

userGroups=$( $EXEC_GROUPS $username )

# Add $username to the 'users' group
if [[ "$userGroups" != *'users'* ]]; then
...
if [ ! -d "/mnt/ramdisk/$username" ]; then
  ...
  $EXEC_CP "$userhome"/.cache/mozilla/* "/mnt/ramdisk/$username/mozilla"
  $EXEC_RM -rf "$userhome"/.cache/mozilla
  $EXEC_LN -s "/mnt/ramdisk/$username/mozilla" "$userhome"/.cache/mozilla
fi
...
if [ ! -d "$userhome"/.ssh ] || \
	[ $($EXEC_FIND "$userhome"/.ssh -type f | $EXEC_WC -l) -eq 0 ]; then
